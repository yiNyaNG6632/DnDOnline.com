import type { ControlScheme, GameState } from './types';

export function updateDropThrough(state: GameState, keys: Set<string>, controls: ControlScheme) {
  ...
  const support = state.platforms.find((platform) => platform.dropThrough !== false
